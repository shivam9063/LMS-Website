...
export const createCourse = async (req, res) => {
    ...
    try {
        const { title, category, isPublished } = req.body
        if (!title || !category) {
            return res.status(400).json({ message: "title and category is required" })
        }

        // determine publish flag - default to true so newly created courses appear publicly
        let publish = true
        if (typeof isPublished !== 'undefined') {
            if (typeof isPublished === 'string') publish = isPublished === 'true'
            else publish = Boolean(isPublished)
        }
...
export const createLecture = async (req, res) => {
    try {
        ...
        // Ensure course exists before creating lecture
        ...
        // Use atomic update to push lecture id to course without calling `course.save()`
        // which would run validators (and fail if course.level is an empty string).
