...
function ViewCourse() {
  ...
  // (modal/autoplay removed) keep selectedLecture for preview playback
...
const handleEnroll = async (courseId, userId) => {
  try {
    ...
    console.log("Starting direct enrollment for:", { courseId, userId });
    
    // Direct enrollment without payment
    const enrollRes = await axios.post(serverUrl + "/api/payment/direct-enroll", {
      courseId,
      userId
    }, {withCredentials: true});
    ...
    setIsEnrolled(true);
    toast.success(enrollRes.data.message);
    // Default behavior: navigate to lecture view after enrollment
    setTimeout(() => navigate(`/viewlecture/${courseId}`), 250);
        ...
        <div className="flex flex-col md:flex-row gap-6 ">
          ...
          <div className="flex-1 space-y-2 mt-[20px]">
            ...
            {!isEnrolled ? (
              ...
              <button
                className="bg-green-200 text-green-600 px-6 py-2 rounded hover:bg-gray-100 hover:border mt-3 cursor-pointer"
                onClick={() => {
                  // open modal with first lecture when user clicks Watch Now
                  // Navigate to the lecture page (keeps original behavior)
